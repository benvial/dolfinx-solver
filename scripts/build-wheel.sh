...
# The same digest, short, for the two stage stamps that name it. petsc4py
# and slepc4py are compiled against the lock's numpy headers and generated
# by its Cython and setuptools; the DOLFINx bindings are configured by its
# scikit-build-core and compiled against its mpi4py's include. A lock bump
# rebuilds the venv above, and without this it would then re-check both
# stages against their existing stamps — publishing extensions built
# against the previous numpy while the "==> build tooling" block in the
# same log names the new one (ticket 29). Twelve hex characters is a cache
# key, not a security claim; the stamp only has to differ when the lock
# does.
#
# The whole lock rather than the part these two compile against: the lock
# is regenerated wholesale by one `uv pip compile`, so a subset would be a
# second list to keep in step with it, and a bump that moves only the
# packaging drivers rebuilds against a warm ccache — the superbuild cache
# restores that even when its exact key misses, which is why the false
# rebuild is minutes rather than the hours a cold stage takes.
tooling_id="${tooling_digest:0:12}"
...
#
# Not stamped on the build venv's lock, unlike the two bindings stages: an
# autotools build by the container's gcc and gfortran out of a source
# tree pinned by digest, with nothing from the venv on its command line
# (ticket 29). The same holds for PETSc, SLEPc, ADIOS2 and KaHIP below.
...
#
# Not stamped on the build venv's lock either (ticket 29): configure runs
# under the venv's interpreter, but as a script runner — every library this
# stage produces is compiled by the container's toolchain from sources
# pinned by digest, and no package in the lock supplies a header, a
# generator or a flag to any of them.
...
# (spec §5, ADR-0001). Not on the build venv's lock, for the reason at the
# PETSc stamp above: this stage compiles against the prefix, not against
# anything the venv holds (ticket 29).
...
#
# Stamped on the build venv's lock, which is what ticket 29 added: both
# extensions take their headers from the lock's numpy and are generated by
# its Cython through setuptools, so a lock bump is an input to their bytes
# in the way a PETSc bump is.
bindings_stamp="$install_prefix/.bindings-petsc-$petsc_version-$scalar_type-slepc-$slepc_version-tooling-$tooling_id.installed"
...
# Not stamped on the build venv's lock: a CMake build by the prefix's MPI
# wrappers, with no package from the venv among its inputs (ticket 29).
...
# The build venv's lock is not in it either, for the same reason as ADIOS2
# above: nothing the venv holds is an input to this CMake build (ticket 29).
...
# Stamped on the build venv's lock, like the bindings stage above: the
# nanobind extension is configured by the lock's scikit-build-core and
