...
recorded in the wheel, which comes from ``dolfinx_solver.__version__``, and a
...
This is that tie. CI runs it on tag pushes, before the multi-hour build, so a
mismatched tag fails in seconds rather than at the upload.

Tags are ``v`` plus the version exactly as the module spells it: ``v0.11.0.post0``
for the DOLFINx release this wheel mirrors, ``v0.11.0.post1`` for a
packaging-only fix on top of it. The comparison is deliberately literal rather
than a parsed-version equality: ``v0.11.0.post01`` and ``v0.11.0.post1`` mean
the same release to Python's version rules but are two different tags to git,
and a release should not be reachable under two spellings.
"""
...
# Read rather than imported: importing ``dolfinx_solver`` loads MPI, and this
# check runs on a tag push before anything has been built. See wheelbuild.version.
...
def tag_problem(tag: str, version: str | None = None) -> str | None:
    ...
    Args:
        ...
        version: Version the package records. Defaults to
            ``dolfinx_solver.__version__``, read when the check runs rather
    ...
    if tagged != version:
        return (
            ...
            "reused once uploaded. Move dolfinx_solver.__version__ or the "
            "tag, whichever is wrong."
        )
    return None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("tag", help="the release tag, such as v0.11.0.post0")
