...
- MPI comes from the PyPI [`mpich`](https://pypi.org/project/mpich/) wheel,
  ...
  older `mpich` nor another MPI — Open MPI, Intel MPI — will do.
...
- Install one scalar variant, never both. The two wheels are the same payload
  at the same paths, so together they overwrite each other's files, and the
  import refuses that environment rather than running whichever landed second.
...
MPI is **not** vendored, so DOLFINx, mpi4py and petsc4py all share one MPI in
one process.
...
chooses its transport through UCX when MPI initialises — which for this wheel
is the import itself — and some hosts expose an RDMA device a guest is not
permitted to open, Azure's MANA card among them. It is not specific to running
in parallel. Name the transports instead of letting UCX discover them:
...
single-machine run needs, serial or under `mpiexec -n N`; the test suite sets
exactly this. **On a cluster with working InfiniBand, do not set it** — there
finding the card is the point. That case is untested: nothing here has ever
run on a machine with a working fabric.
...
`wheeltest/` proves a finished wheel from outside the build: it installs it
into a clean venv and puts five stages to it. Run the whole suite, or one
stage at a time:
...
`WHEELHOUSE` is the directory holding the wheel. `PYTHON` picks the
interpreter the clean venv is made from — the wheel is abi3 and has to work on
3.12, 3.13 and 3.14, and that interpreter needs nothing installed in it.
`DRIVER_PYTHON` is the environment the dev extras are in, and `DEMO_SOURCE` a
DOLFINx source tree to take the demos from instead of downloading the pinned
release.

## Releasing

A `v*` tag publishes three distributions from one workflow run: the two binary
variants it just built and tested, and the bare `dolfinx-solver` meta-package,
built from `meta/`. They go up together — `dolfinx-solver-real` cannot be added
afterwards under the same version, and the meta-package pins its variant
exactly — so the gather step refuses a set that is not all three at
...
per distribution**. A pending publisher — the kind that registers a project
...
name; three projects published from one repository and workflow would be three
identical configurations, and PyPI refuses the second. The environment is the
field left to tell them apart. Register these at
<https://pypi.org/manage/account/publishing/>, owner `benvial`, repository
...
is asserted against it, so the file and the form cannot drift apart. The
GitHub environments already exist and carry no required reviewer; adding one is
what makes that upload pause for a human, which is worth considering given
PyPI never reuses a filename. Nothing can be verified before the fact — PyPI
has no API for reading publishers back — so the first tag is the test.

Noticing that there is something to release is a job of its own:
`.github/workflows/dolfinx-release.yml` asks GitHub weekly what DOLFINx's
latest release is, and when upstream is ahead it writes that release into the
four files that carry it — the version module, the source digest, upstream's
own trio pins and the meta-package — and opens the result as a pull request
for `wheels.yml` to build. It needs a `RELEASE_BUMP_TOKEN` secret, because a
pull request opened with the default token triggers no workflows. The nanobind
coupling, the MPICH series and the tag itself are left to a human, and the
pull request says so.

The tag is checked against the packaged version in the checks job, seconds in
and long before the build:
...
puts three things to it: the meta-package resolves to `dolfinx-solver-complex`,
that wheel solves the numerical smoke problem, and the refusals name the
distribution pip recorded rather than the name that was typed. By hand,
against any index:
